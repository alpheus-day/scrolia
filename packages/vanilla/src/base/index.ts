...
import { MARGIN } from "@scrolia/shared";
...
const createScrollbar = (options?: Options): CreateScrollbar => {
    ...
    const container: CreateContainer = createContainer({ core });
    const content: CreateContent = createContent({ core });
    const trackX: CreateTrackX = createTrackX({ core });
    const trackY: CreateTrackY = createTrackY({ core });
    const thumbX: CreateThumbX = createThumbX({ core });
    const thumbY: CreateThumbY = createThumbY({ core });
