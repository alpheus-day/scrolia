...
import { MARGIN, runByPosition } from "@scrolia/shared";
...
const Container = (props: ContainerProps): React.JSX.Element => {
    ...
    }, [o.headless, o.disabled, o.page]);

    // handle length
    React.useEffect((): (() => void) => {
        const setLength = (): void => {
            runByPosition({
                enableX: hvTrackX && hvThumbX,
                enableY: hvTrackY && hvThumbY,
                fnX: (): void => {
                    let total: number = 0;
                    let view: number = 0;
